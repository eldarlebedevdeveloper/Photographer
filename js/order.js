...
function putInforation(){
		...
		function  adreInformation() {
			$("#order_adresse").validate({
				...
				event.preventDefault();
				   ...
				   paymetData.adresse = document.querySelector('input[name="adresse"]').value
	...
	function debitCardInformation() {
		...
		// cart.style.borderBox = 'none!important'
	...
	function mainFlipButton(idForm){
		...
		console.log(paymetData)
			// document.querySelector('#wpsd_donation_for option').textContent = `9`
			document.querySelector('#wpsd_donator_name').value = `${paymetData.name} ${paymetData.phone} ${paymetData.adresse} ${paymetData.plz} ${paymetData.stadt} ${paymetData.land} `
			document.querySelector('#wpsd_donator_email').value = paymetData.email
			document.querySelector('#wpsd_donate_other_amount').value = paymetData.fullPrice
			document.querySelector('#wpsd_donation_for option').value = paymetData.nameProducts
			document.querySelector('#wpsd_donation_for option').textContent = paymetData.nameProducts		
...
function transfrerBasket() {
	let cloneCart = document.querySelector('.oreder .cart_products').cloneNode(true)
